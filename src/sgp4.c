            ...
            char opsmode )
{
    ...
    if( init == 'n' )
    {
        ...
        //  discontinuity 0.2 rad = 11.45916 deg use next line for original
        //  strn3 approach and original inclination if (inclo >= 0.2) use next
        //  line for gsfc version and perturbed inclination
        if( rec->inclp >= 0.2 )
        {
            ph = ph / sinip;
            pgh = pgh - cosip * ph;
            ...
            ElsetRec * rec )
{
    ...
    for( lsflg = 1; lsflg <= 2; lsflg++ )
    {
        ...
        rec->z11 = -6.0 * a1 * a5 +
                   rec->emsq * ( -24.0 * x1 * x7 - 6.0 * x3 * x5 );
        rec->z12 = -6.0 * ( a1 * a6 + a3 * a5 ) +
                   rec->emsq * ( -24.0 * ( x2 * x7 + x1 * x8 ) -
                                 6.0 * ( x3 * x6 + x4 * x5 ) );
        rec->z13 = -6.0 * a3 * a6 +
                   rec->emsq * ( -24.0 * x2 * x8 - 6.0 * x4 * x6 );
        rec->z21 = 6.0 * a2 * a5 +
                   rec->emsq * ( 24.0 * x1 * x5 - 6.0 * x3 * x7 );
        rec->z22 = 6.0 * ( a4 * a5 + a2 * a6 ) +
                   rec->emsq * ( 24.0 * ( x2 * x5 + x1 * x6 ) -
                                 6.0 * ( x4 * x7 + x3 * x8 ) );
        rec->z23 = 6.0 * a4 * a6 +
                   rec->emsq * ( 24.0 * x2 * x6 - 6.0 * x4 * x8 );
...
void dsinit( double tc, double xpidot, ElsetRec * rec )
{
    ...
    rec->dmdt = sls -
                znl * rec->s3 * ( rec->z1 + rec->z3 - 14.0 - 6.0 * rec->emsq );
    ...
    if( rec->irez != 0 )
    {
        ...
        if( rec->irez == 2 )
        {
            ...
            {
                g211 = -72.099 + 331.819 * rec->em - 508.738 * rec->emsq +
                       266.724 * eoc;
                ...
                g422 = -3581.690 + 16178.110 * rec->em - 24462.770 * rec->emsq +
                       12422.520 * eoc;
                if( rec->em > 0.715 )
                    g520 = -5149.66 + 29936.92 * rec->em -
                           54087.36 * rec->emsq + 31324.56 * eoc;
                else
                    g520 = 1464.74 - 4664.75 * rec->em + 3763.64 * rec->emsq;
            }
            if( rec->em < 0.7 )
            {
                g533 = -919.22770 + 4988.6100 * rec->em -
                       9064.7700 * rec->emsq + 5542.21 * eoc;
                g521 = -822.71072 + 4568.6173 * rec->em -
                       8491.4146 * rec->emsq + 5337.524 * eoc;
                g532 = -853.66600 + 4690.2500 * rec->em -
                       8624.7700 * rec->emsq + 5341.4 * eoc;
            }
            else
            {
                g533 = -37995.780 + 161616.52 * rec->em -
                       229838.20 * rec->emsq + 109377.94 * eoc;
                g521 = -51752.104 + 218913.95 * rec->em -
                       309468.16 * rec->emsq + 146349.42 * eoc;
                g532 = -40023.880 + 170470.89 * rec->em -
                       242699.48 * rec->emsq + 115605.82 * eoc;
            }

            sini2 = rec->sinim * rec->sinim;
            f220 = 0.75 * ( 1.0 + 2.0 * rec->cosim + cosisq );
            f221 = 1.5 * sini2;
            f321 = 1.875 * rec->sinim *
                   ( 1.0 - 2.0 * rec->cosim - 3.0 * cosisq );
            f322 = -1.875 * rec->sinim *
                   ( 1.0 + 2.0 * rec->cosim - 3.0 * cosisq );
            ...
            rec->xlamo = fmod( rec->mo + rec->nodeo + rec->nodeo - theta -
                                   theta,
                               twopi );
            rec->xfact = rec->mdot + rec->dmdt +
                         2.0 * ( rec->nodedot + rec->dnodt - rptim ) -
                         rec->no_unkozai;
            rec->em = emo;
            rec->emsq = emsqo;
        }
        ...
        if( rec->irez == 1 )
        {
            ...
            f311 = 0.9375 * rec->sinim * rec->sinim *
                       ( 1.0 + 3.0 * rec->cosim ) -
                   0.75 * ( 1.0 + rec->cosim );
            f330 = 1.0 + rec->cosim;
            ...
            rec->xlamo = fmod( rec->mo + rec->nodeo + rec->argpo - theta,
                               twopi );
...
void initl( double epoch, ElsetRec * rec )
{
    ...
    d1 = 0.75 * rec->j2 * ( 3.0 * rec->cosio2 - 1.0 ) /
    ...
    c1p2p = c1 + twopi;
    double gsto1 = fmod( thgr70 + c1 * ds70 + c1p2p * tfrac +
                             ts70 * ts70 * fk5r,
                         twopi );
    if( gsto1 < 0.0 )
        gsto1 = gsto1 + twopi;
...
bool sgp4init( char opsmode, ElsetRec * satrec )
{
    ...
    double cc1sq, cc2, cc3, coef, coef1, cosio4, eeta, etasq, perige, pinvsq,
        psisq, qzms24, sfour, tc, temp, temp1, temp2, temp3, tsi, xpidot,
        xhdot1, qzms2t, ss, x2o3, r[ 3 ], v[ 3 ], delmotemp, qzms2ttemp,
        qzms24temp;
    ...
    satrec->am = satrec->em = satrec->im = satrec
                                               ->Om = satrec
                                                          ->mm = satrec
                                                                     ->nm = 0.0;
    ...
    if( ( satrec->omeosq >= 0.0 ) || ( satrec->no_unkozai >= 0.0 ) )
    {
        ...
        satrec->cc4 = 2.0 * satrec->no_unkozai * coef1 * satrec->ao *
                      satrec->omeosq *
        ...
        satrec->argpdot = -0.5 * temp1 * satrec->con42 +
                          0.0625 * temp2 *
                              ( 7.0 - 114.0 * satrec->cosio2 +
                                395.0 * cosio4 ) +
                          temp3 *
                              ( 3.0 - 36.0 * satrec->cosio2 + 49.0 * cosio4 );
        xhdot1 = -temp1 * satrec->cosio;
        satrec->nodedot = xhdot1 +
                          ( 0.5 * temp2 * ( 4.0 - 19.0 * satrec->cosio2 ) +
...
bool sgp4( ElsetRec * satrec, double tsince, double * r, double * v )
{
    double axnl, aynl, betal, cnod, cos2u, coseo1, cosi, cosip, cosisq, cossu,
        ...
        sinsu, sinu, snod, su, t2, t3, t4, tem5, temp, temp1, temp2, tempa,
        tempe, templ, u, ux, uy, uz, vx, vy, vz, xinc, xincp, xl, xlm, xmdf,
        xmx, xmy, nodedf, xnode, tc, x2o3, vkmpersec, delmtemp;
    ...
    xl = satrec->mp + satrec->argpp + satrec->nodep +
         temp * satrec->xlcof * axnl;
...
double gstime( double jdut1 )
{
    ...
    temp = fmod( temp * deg2rad / 240.0, twopi ); // 360/86400 = 1/240, to deg,
                                                  // to rad
